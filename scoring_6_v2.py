""" scoring_6_v2
    High score is now kept. Even when user closes the game, high score is
    written to a file which can be read when the game is re-opened. If the
    User's current score is greater than the previous high score, it can
    write that to the high score file."""
...
def high_score_save(high_score):
    ...
    high_score_document.write(str(high_score))
...
def game_loop():
    ...
    while not quit_game:  # For future use when the 'x' in the corner is used
        while not game_over:
            ...
            if int(score) > int(highscore):  # checking if the user has a new high score
